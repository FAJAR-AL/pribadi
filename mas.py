...
req=reek.Session()
os.system('clear')
title=pyfiglet.figlet_format("MAS FAJAR")
p = "\33[37;1m"
h = "\33[32;1m"
m = "\33[31;1m"
ber=0
...
def bingung():
	print(f"{h}\n[√] KABEH SPAM WES DIKIRIM MAS FAJAR{p}\n")
	print(f"Ingin AREP SPAM MENEH?\nKetik y untuk ya ketik t untuk tidak\n")
	pilih=input("y/t : ")
	if(pilih=="y"):
		cok()
	elif(pilih=="t"):
...
def cok():
	while True:
		try:
			a=input("[+] NOMER E CAH ASU08×××\t: ")
			asu=a[0:2]
			if a in(""," "):print("[!] OJO KOSONG")
			elif "08" not in asu:print("[!] NANGGO NOMER 08xxx\n")
			elif len(a)<=10:print("[!] NOMER KUDU LUWIH SEKO 10 ANGKA MAS")
			elif a=="083883917593":print("[!] Anda Tidak Bisa Spam Yang Punya Script Goblok!\n")
			else:
				try:
					suu=int(input("[+] AREP NYEPAM PIRO\t: "))
				except:
					print("Masukan Format Angka Jangan Huruf!")
					cok()
				time.sleep(2)
				print("\n[+] SABAR MAS LAGI NYEPAM IKI...")
				b=a[1:12] 
				c="62"+b
...
def memek():
	print(f"{p}"+title+f"{h}\nFOLLOW IG @fajar_ap86 | 16 OPRATOR Otp{p}\n\nOJO NGGO ANEH ANEH LHO MAS \nJUMLAH SPAM DIKALEKNE KARO 16\nCONTOH E DIKALI 2\nDADINE 2 DIKALI 16 DADI PIRO\n")
